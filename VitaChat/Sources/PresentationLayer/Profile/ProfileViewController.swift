...
final class ProfileViewController: BaseViewController {
    ...
    // MARK: - Init
    init(output: ProfileViewOutput, currentUser: User) {
        self.output = output
...
//            .vCenter(-100)
    ...
    private func goToChatsTapped() {
        guard
            let sexTitle = sexSegmentedControl.titleForSegment(at: sexSegmentedControl.selectedSegmentIndex),
        ...
        self.output.saveUserData(id: currentUser.uid,
                                 email: email,
                                 contactInfo: contact, imageUrl: "dkdkdk")
    }
